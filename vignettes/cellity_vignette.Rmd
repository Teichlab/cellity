---
title: "Introduction to `cellity`: Single-cell RNA-seq quality control toolkit in R"
...
*`sample training data`: counts, read statistics, labels
*`sample test data`: counts, read statistics, labels
*`training features`: all, common
*`test features`: all, common
...
*sample test data:  a different subset of the original data.
...
features_human = extract_features(sample_counts_nm, sample_stats, common_features = common_features, GO_terms = GO_terms, extra_genes = extra_genes, organism = "human")
```
...
training_quality_PCA_allF=assess_cell_quality_PCA(training_mES_features_all, file="~/training_quality_PCA_allF.pdf")
```
...
training_quality_PCA_commonF=assess_cell_quality_PCA(training_mES_features_common, file="~/training_quality_PCA_commonF.pdf")
```
...
mES1_quality_SVM=assess_cell_quality_SVM(training_mES_features_all, training_mES_labels[,2], param_mES_all, mES1_features_all)
```
...
mES1_quality_SVM_common=assess_cell_quality_SVM(training_mES_features_common, training_mES_labels[,2], param_mES_common, mES1_features_common)
```
...
Of course we won't have this information always, but usually both versions should be in good agreement with each other. So if your data you predict on is the same as the training data (in this case mES) you can probably rely more on the SVM version more and do PCA as a control to see if overall it makes sense. 